...
function StudentLogin() {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      if (response.ok) {
        // FIX: server now always returns data.student
  ...
  const handleMicrosoftLogin = async () => {
    ...
    try {
      ...
      // FIX: set active account immediately after popup
      ...
      if (!userEmail.toLowerCase().endsWith(`@${ALLOWED_DOMAIN}`)) {
        setError(`Only @${ALLOWED_DOMAIN} college accounts are allowed.`);
        setMsLoading(false);
        return;
      }
